...
6. **実装**: 承認後に実装を開始する（**ステップごとにコミットする**）
7. **手順書作成**: 実装内容の手順書を `manual/<画面名>/` に保存する
8. **ドキュメント作成**: 実装完了後のドキュメントを `docs/<画面名>/` に保存する
9. **PR作成**: PRタイトルとPR文を作成し、`develop` に向けてPull Requestを作成する

**承認なしに実装を開始してはならない。**

### コミットルール

- 実装はステップごとに細かくコミットする（1つの大きなコミットにまとめない）
- 各コミットは1つの論理的な変更単位とする
- コミットメッセージはGitmoji規約に従う

### PR作成ルール

- PRタイトル: 簡潔に変更内容を要約する（Gitmoji + 日本語）
- PR文: 以下のフォーマットに従う
